...
import { T, join, map, zipWith } from "ramda";
...
export type ProcExp = { tag: "ProcExp"; args: VarDecl[], body: CExp[]; returnTE: TExp; TypePred :TExp; }
export const makeProcExp = (args: VarDecl[], body: CExp[], returnTE: TExp , TypePred:TExp ): ProcExp =>
    ({ tag: "ProcExp", args: args, body: body, returnTE: returnTE , TypePred : TypePred});
export const isProcExp = (x: any): x is ProcExp => x.tag === "ProcExp";

export type TypepredExp = {tag: "TypepredExp"; paramTE: TExp; returnTE: TExp;}
export const makeTypepredExp = (paramTE: TExp, returnTE: TExp): TypepredExp =>
    ({ tag: "TypepredExp", paramTE: paramTE, returnTE: returnTE});
...
export type PrimOpKeyword = "+" | "-" | "*" | "/" | ">" | "<" | "=" | "not" | "and" | "or" | "eq?" | "string=?" |
    "cons" | "car" | "cdr" | "list" | "pair?" | "list?" | "number?" | "boolean?" | "symbol?" | "string?" | "union?" | "inter?" |
    "display" | "newline";
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const returnTE = rest[0] === ":" ? rest[1] === "is?" ? parseTExp([rest[1],rest[2]]) : parseTExp(rest[1]) : makeOk(makeFreshTVar());//if there is "is?" then the return type is Boolean and we have to update the typePredicate"
        if(rest[1] === "is?"){ 
            return bind(args, (args: VarDecl[]) =>
                bind(body, (body: CExp[]) =>
                    mapv(returnTE, (returnTE: TExp) =>
                        makeProcExp(args, body, makeBoolTExp() , returnTE))));
        }
        return bind(args, (args: VarDecl[]) =>
            bind(body, (body: CExp[]) =>
                mapv(returnTE, (returnTE: TExp) =>
