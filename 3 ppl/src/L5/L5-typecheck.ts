// L5-typecheck
// ========================================================
import { equals, includes, map, zipWith } from 'ramda';
...
import {isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp, makeAnyTExp, makeNeverTExp, isAnyTExp , isNeverTExp ,
         ...
         makeInterTExp,
         NeverTExp,
         containsType , makeTExp, ProcTExp,
         InterTExp,
         UnionTExp} from "./TExp";
import { isEmpty, allT, first, rest, NonEmptyList, List, isNonEmptyList } from '../shared/list';
import { Result, makeFailure, bind, makeOk, zipWithResult, either, mapResult, isOk } from '../shared/result';
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isAnyTExp(exp) ? makeOk(makeAnyTExp()) :
    isNeverTExp(exp) ? makeOk(makeNeverTExp()) :
...
export const typeofPrim = (p: PrimOp): Result<TExp> =>
    ...
    (p.op === 'inter?') ? parseTE('(T -> boolean)') :
    (p.op === 'union?') ? parseTE('(T -> boolean)') :
...
const isTypePredApp = (e: Exp, tenv: TEnv): Result<{ pred: CExp; arg: CExp }> => {
    if ((isAppExp(e) && isPrimOp(e.rator) && ['number?', 'boolean?', 'string?'].includes(e.rator.op)) ||(isAppExp(e) && isVarRef(e.rator))){
        if (e.rands.length === 1) {
            return makeOk({ pred: e.rator, arg: e.rands[0] });
        } else {
            return makeFailure(`Type predicate ${e.rator} applied to incorrect number of arguments`);
        }
    }
    return makeFailure('Not a type predicate application');
};

export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> => {
    const typePredResult = isTypePredApp(ifExp.test, tenv);

    if (isOk(typePredResult)) {
        const { pred, arg } = typePredResult.value;
        // If the test is a type predicate, check the types of then and else branches
        const thenTE = typeofExp(ifExp.then, tenv);
        const altTE = typeofExp(ifExp.alt, tenv);
        const predTE = typeofExp(pred, tenv);
        const argTE = typeofExp(arg, tenv);
        const predtype = TypePredResult(predTE , tenv);
        return bind(argTE, (t1: TExp) =>
            bind(predtype, (t2: TExp) =>
                // Ensure both branches have compatible types
                bind(checkCompatibleType(t1, t2 ,ifExp), _ =>
                    makeOk(t2)
                )
            )
        );
...
//proc.pretType
const TypePredResult = (proc : any, tenv: TEnv): Result<TExp> => {
    if(isProcTExp(proc.value)){
    if (proc.value.TypePred.tag === "NeverTExp") {
        return makeOk(makeNeverTExp());
    } else {
        return makeOk(proc.value.TypePred);
    }
}
    return makeFailure(`Type predicate is not compatible with the arguments types`);}
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    ...
    if(proc.TypePred.tag === "NeverTExp"){
        const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) =>
            checkCompatibleType(body, proc.returnTE, proc));
        return bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, proc.returnTE)));
    }else{
        const constraint1 = bind(typeofExps(proc.body, extTEnv), (body: TExp) =>
            checkCompatibleType(body, makeBoolTExp(), proc));
        return (isNumTExp(proc.TypePred)) ? bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeNumTExp()))) :
            (isBoolTExp(proc.TypePred)) ? bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeBoolTExp()))) :
                (isStrTExp(proc.TypePred) ) ? bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeBoolTExp()))) :
                    (isUnionTExp(proc.TypePred)) ? UnionTypePred(proc.TypePred, tenv, argsTEs, proc, constraint1) : 
                     ( isInterTExp(proc.TypePred)) ? InterTypePred(proc.TypePred, tenv, argsTEs, proc, constraint1) :
                    (isAnyTExp(proc.TypePred) ) ? bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeAnyTExp()))) :
                        (isVoidTExp(proc.TypePred) ) ? bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeVoidTExp()))) :
                            makeFailure(`Type predicate is not compatible with the arguments types ----- here`);
}};

const InterTypePred = (exp : InterTExp, tenv: TEnv , argsTEs : TExp[] , proc : ProcExp , constraint1 : Result<true> ): Result<TExp> => {
    return bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeInterTExp([exp.components[0], exp.components[1]]))))
}
const UnionTypePred = (exp : UnionTExp, tenv: TEnv , argsTEs : TExp[] , proc : ProcExp , constraint1 : Result<true> ): Result<TExp> => {
    return bind(constraint1, _ => makeOk(makeProcTExp(argsTEs, proc.returnTE, makeUnionTExp([exp.components[0], exp.components[1]]))))
}
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        ...
        const constraints = zipWithResult(
            (rand, trand) => bind(typeofExp(rand, tenv), (typeOfRand: TExp) => checkCompatibleType(typeOfRand, trand, app)),
            app.rands,
            ratorTE.paramTEs
        );
...
// Helper function to determine if an expression is a type predicate
const isTypePredicate = (exp: CExp): boolean => {
    if (isPrimOp(exp)) {
        const op = exp.op;
        return op === 'number?' || op === 'boolean?' || op === 'string?' || op === 'union?' || op === 'inter?';
    }
    return false;
};
...
export const typeofLetrec = (exp: LetrecExp, tenv: TEnv): Result<TExp> => {
    ...
    const tenvBody = makeExtendTEnv(ps, zipWith((tij, ti) => makeProcTExp(tij, ti ,makeBoolTExp()), tijs, tis), tenv);
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): Result<VoidTExp> =>
    bind(typeofExp(exp.val, makeExtendTEnv([exp.var.var], [makeTExp(exp.val)], tenv)),
        _ => makeOk(makeVoidTExp()));
