...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | ClassExp;//here
...
export type ClassExp = { tag: "ClassExp"; fields: VarDecl[]; methods: Binding[]; }//here


// ========================================================
export type InstanceExp = { tag: "InstanceExp", classExp: CExp, args: CExp[] };
export type MethodCallExp = { tag: "MethodCallExp", obj: CExp, method: string, args: CExp[] };
export const makeInstanceExp = (classExp: CExp, args: CExp[]): InstanceExp =>
    ({ tag: "InstanceExp", classExp, args });
export const makeMethodCallExp = (obj: CExp, method: string, args: CExp[]): MethodCallExp =>
    ({ tag: "MethodCallExp", obj, method, args });
// ========================================================
...
export const makeClassExp = (fields: VarDecl[], methods: Binding[]): ClassExp =>
    ({ tag: "ClassExp", fields: fields, methods: methods });//here
...
export const isClassExp = (x: any): x is ClassExp => x.tag === "ClassExp";//here

export const isInstanceExp = (x: any): x is InstanceExp => x.tag === "InstanceExp";
export const isMethodCallExp = (x: any): x is MethodCallExp => x.tag === "MethodCallExp";
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isClassExp(x);//here
...
const isSpecialForm = (x: string): boolean =>
    ["if", "lambda", "let", "quote", "class"].includes(x);//here
...
export const parseClassExp = (varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {//here

    if (!Array.isArray(varDecls) || !allT(isString, varDecls))
        return makeFailure("Invalid fields decleration");
    if (!isGoodBindings(bindings))
        return makeFailure("Invalid methods declerations");
...
const unparseClassExp = (cl: ClassExp): string =>
    `(class (${map((f: VarDecl) => f.var, cl.fields).join(" ")}) (${map((b: Binding) => `(${b.var.var} ${unparseL3(b.val)})`, cl.methods).join(" ")}))`//here
                                        ...
                                        isLetExp(exp) ? unparseLetExp(exp) :
                                            isDefineExp(exp) ? `(define ${exp.var.var} ${unparseL3(exp.val)})` :
                                                isProgram(exp) ? `(L3 ${unparseLExps(exp.exps)})` :
                                                    isClassExp(exp) ? unparseClassExp(exp) ://here
                                                        exp;
